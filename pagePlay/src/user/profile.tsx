...
import { BookOpen, Calendar, Camera, Gift, Users } from "lucide-react";
...
const ProfilePage: React.FC = () => {
  ...
  const [gerers, setGenres] = useState<string[]>([]);
    ...
    const fetchTransactions = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/user/transactions?userId=${userId}`
        );
        if (!response.ok) {
  ...
  //     try{
  //       const response = await fetch(
  //         `http://localhost:5000/user/profileRemaining?userId=${userId}`
  //       );
  //       if (!response.ok) {
