...
const SubmitPosterPage: React.FC = () => {
  ...
  const fileUpload = async (file: File) => {
    ...
    const { data: publicUrlData } = supabase.storage.from("posters_img").getPublicUrl(fileName);
    ...
    <div className="min-h-screen bg-[#E5EADD] py-12 px-4">
      ...
      <div className="mt-8">
        <Card className="max-w-2xl mx-auto p-6 bg-gradient-to-br from-[#FAF7ED] to-[#E5EADD] shadow-lg rounded-xl">
          <h1 className="text-3xl font-bold text-[#265073] mb-6 text-center">Submit Your Poster</h1>
          ...
          {submitMessage && (
            <p className="mt-4 text-center font-semibold text-[#265073]">{submitMessage}</p>
          )}
        </Card>
      </div>
    </div>
